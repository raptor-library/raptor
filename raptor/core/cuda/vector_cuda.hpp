...
#ifndef RAPTOR_CORE_VECTOR_CUDA_HPP_
#define RAPTOR_CORE_VECTOR_CUDA_HPP_

#include "core/types.hpp"
#include <cuda_runtime_api.h>
#include <cuda.h>

// Vector Class for Cuda implementation
//
...
public:
    ...
    **************************************************************/
    ...
    void resize(int len)
    {
        ...
        num_values = len;
	// free dev_ptr if it's on device, then allocate memory
	if (dev_ptr) cudaFree(dev_ptr);
	cudaMalloc((void**)&dev_ptr, num_values * b_vecs * sizeof(double));
	// reset gpu launch parameters
        blocksize = 512;
	tblocks = (num_values * b_vecs) / blocksize + 1;
    }
    ...
    **************************************************************/
    ...
    void copy_from_device(); // copy to device function
    void copy_from_host(); // copy from device function
    ...
    // Additional members for cuda control
    double *dev_ptr;
    index_t tblocks;
    index_t blocksize;
    // cuda grid size
    // cuda block size - could just set this at runtime? make default to be b_vecs, unless
    // 			 b_vecs is larger than a warp size, then calculate optimal
};
