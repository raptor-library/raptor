...
CSRMatrix* CommPkg::communicate(ParCSRMatrix* A, const bool has_vals)
{
    ...
    return communicate(rowptr, col_indices, values, A->on_proc->b_rows, 
            A->on_proc->b_cols, has_vals);
}
        ...
        const int b_rows, const int b_cols, const bool has_vals)
{
    CSRMatrix* recv_mat = communication_helper(rowptr.data(), col_indices.data(), values.data(),
            send_data, recv_data, key, mpi_comm, b_rows, b_cols, has_vals);
    key++;
    return recv_mat;
}
CSRMatrix* ParComm::communicate(const aligned_vector<int>& rowptr, 
        ...
        const int n_result_rows, const int b_rows, const int b_cols, const bool has_vals)
{
    CSRMatrix* recv_mat_T = communication_helper(rowptr.data(), col_indices.data(), 
            values.data(), recv_data, send_data, key, mpi_comm, b_rows, b_cols, has_vals);

    CSRMatrix* recv_mat = transpose_recv(recv_mat_T, recv_mat_T->vals, 
...
CSRMatrix* ParComm::communicate_T(const aligned_vector<int>& rowptr, 
        ...
        const int n_result_rows, const int b_rows, const int b_cols, const bool has_vals)
{
    BSRMatrix* recv_mat_T = (BSRMatrix*) communication_helper(rowptr.data(), 
            col_indices.data(), values.data(), recv_data, send_data, key, 
            mpi_comm, b_rows, b_cols, has_vals);

    BSRMatrix* recv_mat = (BSRMatrix*) transpose_recv(recv_mat_T, recv_mat_T->block_vals, 
            send_data, n_result_rows);
...
{   
    ...
    if (local_S_par_comm)
    {
        ...
        G_mat = global_par_comm->communicate(S_mat->idx1, S_mat->idx2, 
                S_mat->vals, b_rows, b_cols, has_vals);
        delete S_mat;
    }
    else
    {
    ...
    CSRMatrix* recv_mat = combine_recvs(L_mat, R_mat, 
...
CSRMatrix* TAPComm::communicate(const aligned_vector<int>& rowptr, 
        ...
        const int b_rows, const int b_cols, const bool has_vals)
{   
    BSRMatrix* L_mat = (BSRMatrix*) local_L_par_comm->communicate(rowptr, col_indices,
            values, b_rows, b_cols, has_vals);
    ...
    if (local_S_par_comm)
    {
        BSRMatrix* S_mat = (BSRMatrix*) local_S_par_comm->communicate(rowptr, col_indices, values, 
                b_rows, b_cols, has_vals);
        G_mat = (BSRMatrix*) global_par_comm->communicate(S_mat->idx1, S_mat->idx2, 
                S_mat->block_vals, b_rows, b_cols, has_vals);
        delete S_mat;
    }
    else
    {
        G_mat = (BSRMatrix*) global_par_comm->communicate(rowptr, col_indices, values, 
                b_rows, b_cols, has_vals);
    }

    BSRMatrix* R_mat = (BSRMatrix*) local_R_par_comm->communicate(G_mat->idx1, G_mat->idx2, 
            G_mat->block_vals, b_rows, b_cols, has_vals);
    delete G_mat;

    // Create recv_mat (combination of L_mat and R_mat)
    CSRMatrix* recv_mat = combine_recvs(L_mat, R_mat, 
            L_mat->block_vals, R_mat->block_vals, b_rows, b_cols,
            (NonContigData*) local_L_par_comm->recv_data,
            (NonContigData*) local_R_par_comm->recv_data,
            get_buffer<int>());
...
{   
    ...
    CSRMatrix* G_mat = communication_helper(R_mat->idx1.data(), R_mat->idx2.data(),
            ...
            global_par_comm->key, global_par_comm->mpi_comm, b_rows, b_cols, has_vals);
    global_par_comm->key++;
    delete R_mat;
    ...
    CSRMatrix* recv_mat = combine_recvs_T(L_mat, final_mat,
...
{   
    ...
    BSRMatrix* L_mat = (BSRMatrix*) communication_helper(rowptr.data(), col_indices.data(), 
            values.data(), local_L_par_comm->recv_data, 
            local_L_par_comm->send_data, local_L_par_comm->key,
            local_L_par_comm->mpi_comm, b_rows, b_cols, has_vals);
    local_L_par_comm->key++;
    ...
    BSRMatrix* G_mat = (BSRMatrix*) communication_helper(R_mat->idx1.data(), R_mat->idx2.data(),
            R_mat->block_vals.data(), global_par_comm->recv_data, global_par_comm->send_data,
            global_par_comm->key, global_par_comm->mpi_comm, b_rows, b_cols, has_vals);
    global_par_comm->key++;
    delete R_mat;
    ...
    if (local_S_par_comm)
    {
        final_mat = (BSRMatrix*) communication_helper(G_mat->idx1.data(), G_mat->idx2.data(),
                G_mat->block_vals.data(), local_S_par_comm->recv_data, local_S_par_comm->send_data, 
                local_S_par_comm->key, local_S_par_comm->mpi_comm, b_rows, b_cols, has_vals);
        local_S_par_comm->key++;
        delete G_mat;
        final_comm = local_S_par_comm;
    ...
    CSRMatrix* recv_mat = combine_recvs_T(L_mat, final_mat,
            local_L_par_comm->send_data, final_comm->send_data,
            L_mat->block_vals, final_mat->block_vals, n_result_rows, b_rows, b_cols);
