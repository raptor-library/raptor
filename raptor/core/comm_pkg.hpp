    ...
    namespace raptor
    {
                    ...
                    for (int i = 0; i < send_data->num_msgs; i++)
                    {
                        ...
                        for (int j = 0; j < (end-start); j++)
                        {
                            ...
                            for (int b = 0; b < block_size; b++)
                            {
                                for (int v = 0; v < vblock_size; v++)
                                {
                                    result[v*result_vec_offset + idx + b]  = result_func(result[v*result_vec_offset + idx + b], sendbuf[v*(end-start) + pos + b]);
                                }
                            }
                        }
                    }
        ...
        **************************************************************/
        ...
        void communicate_T(const aligned_vector<T>& values, aligned_vector<U>& result,
                ...
                T init_result_func_val = 0)
        {
            CommPkg::communicate_T(values.data(), result, block_size, vblock_size, vblock_offset, result_func, init_result_func,
                    init_result_func_val);
        }
        template<typename T, typename U>
        void communicate_T(const T* values, aligned_vector<U>& result,
                ...
                T init_result_func_val = 0)
        {
            CommPkg::communicate_T(values, result, block_size, vblock_size, vblock_offset, result_func, init_result_func,
                    init_result_func_val);
        }
        template<typename T>
        void communicate_T(const aligned_vector<T>& values,
                ...
                T init_result_func_val = 0)
        {
            CommPkg::communicate_T(values.data(), block_size, vblock_size, vblock_offset, init_result_func, init_result_func_val);
        }
        template<typename T>
        void communicate_T(const T* values, const int block_size = 1, const int vblock_size = 1,
                const int vblock_offset = 0,
                ...
                T init_result_func_val = 0)
        {
            ...
            // Initial redistribution among node
            local_R_par_comm->communicate_T(values, block_size, vblock_size, vblock_offset, init_result_func, init_result_func_val);

            // Begin inter-node communication 
            aligned_vector<T>& R_sendbuf = local_R_par_comm->send_data->get_buffer<T>();
            global_par_comm->init_comm_T(R_sendbuf, block_size, vblock_size, vblock_offset, init_result_func, init_result_func_val);
        }
        ...
        {
            // Complete inter-node communication
            global_par_comm->complete_comm_T<T>(block_size, vblock_size, vblock_offset, init_result_func, init_result_func_val);
