...
*****    Nonzero value to be inserted into the matrix
**************************************************************/
void CSRBoolMatrix::copy(const COOMatrix* A)
{
    n_rows = A->n_rows;
    n_cols = A->n_cols;
    nnz = A->nnz;
...
void CSRBoolMatrix::copy(const CSRMatrix* A)
{
    n_rows = A->n_rows;
    n_cols = A->n_cols;
    nnz = A->nnz;
...
void CSRBoolMatrix::copy(const CSCMatrix* A)
{
    n_rows = A->n_rows;
    n_cols = A->n_cols;
    nnz = A->nnz;
...
void CSRBoolMatrix::sort()
{
    ...
    for (int row = 0; row < n_rows; row++)
    {
        ...
        std::sort(idx2.begin() + start, idx2.begin() + end);
    }
...
void CSRBoolMatrix::remove_duplicates()
{
    int orig_start, orig_end;
    int new_start;
    int col, prev_col;
