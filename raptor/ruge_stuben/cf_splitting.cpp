...
void rs_first_pass(const ParCSRMatrix* S,
        ...
        std::vector<int>& states)
{
    ...
    if (S->local_num_rows)
    {
        states.resize(S->local_num_rows, -1);
    }
    while (remaining)
    {
        // Select vertex with max weight
        max_weight = 0;
        max_idx = 0;
        for (int i = 0; i < S->local_num_rows; i++)
        {
            if (states[i] == -1 && weights[i] > max_weight)
            {
        ...
        // Add max weight vertex to coarse
        states[max_idx] = 1;
        remaining--;

        // Add all row-wise neighbors and dist2 neighbors to fine
        start = S->on_proc->idx1[max_idx];
        end = S->on_proc->idx1[max_idx+1];
        for (int j = start; j < end; j++)
        {
            ...
            if (states[idx] == -1)
            {
                ...
                // Add one to weight of all unassigned neighbors of idx
        ...
        // Add all col-wise neighbors and dist2 neighbors to fine
        start = col_ptr[max_idx];
        end = col_ptr[max_idx+1];
        for (int j = start; j < end; j++)
        {
            idx = col_indices[j];
            if (states[idx] == -1)
...
void rs_second_pass(const ParCSRMatrix* S, 
        std::vector<int> states)
{
    int head, length;
    int idx, idx_k, k;
    ...
    // no shared coarse point
    for (int i = 0; i < S->local_num_rows; i++)
    {
        if (states[i] == 1) continue;

        // If i is fine, find all coarse points in row
        head = -2;
        length = 0;
        ...
        // Find all fine points in row
        for (int j = start; j < end; j++)
        {
            idx = S->on_proc->idx2[j];
            if (states[idx] == 0) // fine point
            {
                // See if fine row contains any same coarse points as i
                idx_start = S->on_proc->idx1[idx];
                idx_end = S->on_proc->idx1[idx+1];
                ...
                // If no shared coarse points, set col as coarse
                if (k == idx_end)
                {
                    states[idx] = 1;
                    
                    // Add to row_coarse for checks on future rows
                    row_coarse[idx] = 1;
                    next[idx] = head;
                    head = idx;
                    length++;
        ...
        std::vector<double>& weights)
{
    ...
    for (int i = 0; i < S->local_num_rows; i++)
    {
        ...
        {
            idx = S->on_proc->idx2[j];
            if (S->on_proc->vals[j] == 1 && dist2[idx])
            {
                weights[idx] -= 1;
                S->on_proc->vals[j] = -1;
            }
        ...
        std::vector<int>& off_proc_weight_updates)
{
    ...
    for (int i = 0; i < S->local_num_rows; i++)
    {
        ...
        for (int j = start; j < end; j++)
        {
            ...
            if (off_proc_states[idx] == -1 && S->off_proc->vals[j] == 1)
            {
        ...
        std::vector<int>& off_proc_states)
{
    ...
    while (off_remaining)
    {
        ...
        update_weights_offproc_dist2(S, off_proc_col_ptr, off_proc_col_coarse,
                states, off_proc_states, off_proc_weight_updates);
        ...
        int remaining)
{
    ...
    if (S->off_proc_num_cols)
    {
        ...
        off_proc_weights.resize(S->off_proc_num_cols, 0);
        off_proc_states.resize(S->off_proc_num_cols, -1);
    }
    if (S->comm->send_data->size_msgs)
    {
        ...
        std::vector<int>& states)
{
    ...
    /**********************************************
     * Set weights for initial Ruge-Stuben coarsening
     **********************************************/
    for (int i = 0; i < S->local_num_rows; i++)
    {
        start = S->on_proc->idx1[i];
     ...
     **********************************************/
    rs_first_pass(S, on_col_ptr, on_col_indices,
            weights, states);
    rs_second_pass(S, states);
